...
- Personagens, casas e hotel usam os modelos GLB fornecidos e renderização WebGL local. Cada pessoa escolhe um dos 12 personagens antes de entrar; o servidor impede dois jogadores de usarem o mesmo modelo. Os modelos dependem de `public/models/Textures/colormap.png`; enquanto a textura original não estiver disponível, o jogo aplica cores provisórias. O arquivo recebido como `Dados.glb` contém internamente um modelo chamado `Gambling Icon`; por isso, o jogo gera dois dados cúbicos 3D no próprio Three.js, com lançamento e resultado, até a inclusão de um GLB de dado correto.
