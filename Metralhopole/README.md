...
- Personagens, casas, hotel e o par de dados usam os modelos GLB fornecidos e renderização WebGL local. Cada pessoa escolhe um dos 12 personagens antes de entrar; o servidor impede dois jogadores de usarem o mesmo modelo. Personagens e construções dependem de `public/models/Textures/colormap.png`; enquanto a textura original não estiver disponível, o jogo aplica cores provisórias. Os dois dados vêm juntos no modelo `Dices.glb` e são animados como uma única peça visual a cada lançamento.
